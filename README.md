...
- **高安全性**:
    - **多层级防爆破**: 系统集成渐进式防御机制。失败 3 次起触发强制响应延迟，等待时长随失败次数稳步递增（最高 300s），有效对抗暴力破解攻击。
    - **智能指纹识别**: 针对内网穿透（如 frp）等复杂网络环境，系统结合 `IP + User-Agent` 进行细粒度指纹校验。即使多个用户共用同一穿透 IP，也能精准隔离攻击者，避免误伤正常登录。
    - **前端实时拦截**: 触发惩罚时，登录按钮将自动锁定并显示精确到秒的倒计时，且即便刷新页面防御依然有效。
    - **深度审计日志**: 自动记录 `login_errors.log`，包含攻击者的 IP、User-Agent 指纹、失败次数及触发的惩罚时长，方便管理员复查安全事件。
    - **生产级适配**: 支持识别 `X-Forwarded-For` 真实 IP，完美兼容 Nginx、Cloudflare 等反向代理环境。
    - **读写优化**: 数据库采用 WAL (Write-Ahead Logging) 模式，确保在大规模历史数据存储与实时访问时的极致流畅。
![alt text](/img/login_error.png)
